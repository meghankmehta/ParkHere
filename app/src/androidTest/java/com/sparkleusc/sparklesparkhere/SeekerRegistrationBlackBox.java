...
@LargeTest
public class SeekerRegistrationBlackBox {

    @Rule
    public ActivityTestRule<SignUpActivity> mActivityRule = new ActivityTestRule(SignUpActivity.class);

    @Test
    public void RegisterSeekerTest(){
        onView(withId(R.id.FirstNameET)).perform(typeText("SEEKERBEEKER"), closeSoftKeyboard());

        onView(withId(R.id.LastNameET)).perform(typeText("BEEKERSEEKER"), closeSoftKeyboard());
        ...
        onView(withId(R.id.rbSeeker)).check(matches(isChecked()));
